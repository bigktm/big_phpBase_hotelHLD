...
if (isset($_SESSION['currUser'])){
	$GLOBALS['userId'] = $_SESSION['currUser']['id'];
}
...
function update_session()
{
	...
	if($userId){
		$sql = mysql_query("SELECT * FROM user WHERE id =".$userId." AND type = 1");
...
function get_userdn($email,$password)
{
	...
	$sql = mysql_query("SELECT * FROM user WHERE email ='".$email."' AND password = '".$password."' AND type = 1");
	if(mysql_num_rows($sql)==1)
	{
		$row = mysql_fetch_array($sql);
		$_SESSION['currUser'] = $row;
		...
		echo "<div class='alert alert-block alert-danger fade in'>Tên đăng nhập hoặt mật khẩu sai.</div>";
	}
}

function register_user($name,$email,$password)
{
	$query = "INSERT INTO user(id, name, email, password, type, hotelId, avatar) VALUES ('','".$name."','".$email."','".$password."',1,1,'userd.jpg')";
	$sql = mysql_query($query);
	if ($sql == 1){
		header("location:login.php?create=success");
	}else {
		echo "<div class='alert alert-block alert-danger fade in'>Error!". mysql_error(). "</div>";
	}
}
...
function get_news()
{
	$query = "select new.id, new.title, new.content, new.createDate, category.name, new.categoryId from new left join category on new.categoryId = category.id";
	$sql = mysql_query($query);
	return $sql;
}

function get_rooms($roomTypeId,$status)
{
	$query = "select room.id, room.label, room.status, room.description, room.roomtypeId, roomtype.name from room left join roomtype on room.roomTypeId = roomtype.id where room.roomTypeId = ". $roomTypeId." AND status =".$status;
	$sql = mysql_query($query);
	return $sql;
}
